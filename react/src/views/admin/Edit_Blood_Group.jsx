...
export default function Edit_Blood_Group(props) {
    ...
    const fetchBlood= () =>{
        http.get('/blood_groups/'+id+'/edit').then((res)=>{
            setInputs({
    ...
    const submitForm = () =>{
        http.put('/blood_groups/'+id, { ...inputs}).then((res)=>{
            navigate('/admin/blood_bank');
        })
    }
    return (
        <div>
  ...
  <Link to='/blood_bank'>Back</Link>
</button>
            <h1 class="px-6 py-4 text-center text-2xl"><b>Update {[inputs.blood_group, inputs.Rh]}'s Quantity</b></h1>
            <div className="row">
